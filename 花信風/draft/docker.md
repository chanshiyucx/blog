...
# 安装 openssh-server（似乎ubuntu自带已安装）
...
由于 Docker 仅支持 window10 专业版系统，或是已安装 Docker for Windows 才能运行。我自用的小米 Pro 笔记本系统是出厂自带的 Win10 家庭版，然而官方的 Docker Desktop 无法在 Win10 家庭版上安装，安装程序会在安装的时候检测系统版本，直接安装会显示安装失败。所以需要通过修改注册表的方式绕过安装检测，才能正常安装。

使用管理员权限运行以下命令，之后便可安装 Docker for Windows：
